...
:	m_cstrRegKey( lpszRegKey ),
	m_cstrRegValueFormat( lpszValueFormat ),
	m_nMaxMRUSize( nMaxMRUSize ),
	m_bSaveOnDestroy( bAutoSaveOnDestroy ),
	m_bSaveAfterAdd( bAutoSaveAfterAdd ),
	m_bRefreshAfterAdd( bAutoRefresh ),
    ...
    try
        ...
        if ( NULL != m_pMRU )
            ...
            delete m_pMRU;
                                        ...
                                        // and make a new one!
        // Sven Wiegand: changed the constructor call, so that it matches
        // CMRUStringList
        m_pMRU = new CMRUStringList ( m_cstrRegKey,
