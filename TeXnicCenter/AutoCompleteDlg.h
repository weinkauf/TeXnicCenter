...
#define MINIMUM_KEYWORD_LENGTH 2
...
public:
	...
	/** 
		...
		*/
	BOOL InitWithKeyword(CString &keyword);
...
public:
	...
	/**
		Called, if user pressed backspace, the edit window should delete the current char to sync with the
		listbox selection
		*/
	virtual void OnACBackspace() = 0;
	/**
		Called, if user typed a char (see documentation of WM_CHAR message for details). This event may be passed
