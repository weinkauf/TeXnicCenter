...
void CCFileViewEx::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (m_bLDragging)
	{
		...
		if (!m_bCopyDrag)
		{
			DeleteItem(CTreeCursor(m_htiDrag, this));
			m_bCopyDrag = FALSE;
		}
		SelectItem(htiNew);
	}
...
BOOL CCFileViewEx::CanDeleteItem(HTREEITEM hItem)
{
	...
	{
		TRACE0(_T("Deletion of this item is not allowed!\n"));
...
void CCFileViewEx::OnFileDelete() 
{
	...
	if (CanDeleteItem(m_ItemSel))
		DeleteItem(CTreeCursor(m_ItemSel, this));		
}
...
{
	AfxMessageBox("Now going to update output files");
	// TODO: Add your command handler code here
