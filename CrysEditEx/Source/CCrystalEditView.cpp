...
/********************************************************************
 *
 * $RCSfile: CCrystalEditView.cpp,v $
 * $Revision$
 * $Date$
 *
 * $Author$
 *
 * $Log: CCrystalEditView.cpp,v $
 * Revision 1.8  2005/06/08 22:58:22  owieland
 * Handle Ctrl+Space
 *
...
BOOL CCrystalEditView::DoDropText(COleDataObject *pDataObject, const CPoint &ptClient)
{
	...
	LPTSTR pszText = static_cast<LPTSTR>(::GlobalLock(hData));
...
void CCrystalEditView::OnEditOperation(int nAction, LPCTSTR pszText)
{
	...
	if (m_bAutoIndent)
	{
		...
		if (nAction == CE_ACTION_TYPING && _tcscmp(pszText, _T("\r\n")) == 0 && !m_bOvrMode)
		{
			...
			int nPos = 0;
			while (nPos < nLength && CharTraitsT::IsSpace(pszLineChars[nPos]))
				nPos++;
