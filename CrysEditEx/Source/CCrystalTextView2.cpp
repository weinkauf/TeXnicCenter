...
/********************************************************************
 *
 * $RCSfile: CCrystalTextView2.cpp,v $
 * $Revision$
 * $Date$
 *
 * $Author$
 *
 * $Log: CCrystalTextView2.cpp,v $
 * Revision 1.10  2007/01/15 18:50:40  niteria
...
void CCrystalTextView::MoveWordLeft(BOOL bSelect)
{
	...
	while (nPos > 0 && CharTraitsT::IsSpace(pszChars[nPos - 1]))
		nPos--;
	...
	{
		nPos--;
		if (CharTraitsT::IsAlnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
		{
			while (nPos > 0 && (CharTraitsT::IsAlnum(pszChars[nPos - 1]) || pszChars[nPos - 1] == _T('_')))
				nPos--;
		}
		else
		{
			while (nPos > 0 && !CharTraitsT::IsAlnum(pszChars[nPos - 1])
			        && pszChars[nPos - 1] != _T('_') && !CharTraitsT::IsSpace(pszChars[nPos - 1]))
				nPos--;
		}
	}
...
void CCrystalTextView::MoveWordRight(BOOL bSelect)
{
	...
	if (CharTraitsT::IsAlnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
	{
		while (nPos < nLength && CharTraitsT::IsAlnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
			++nPos;
	}
	else
	{
		while (nPos < nLength && !CharTraitsT::IsAlnum(pszChars[nPos])
		        && pszChars[nPos] != _T('_') && !CharTraitsT::IsSpace(pszChars[nPos]))
			++nPos;
	}

	while (nPos < nLength && CharTraitsT::IsSpace(pszChars[nPos]))
		++nPos;
...
void CCrystalTextView::MoveHome(BOOL bSelect)
{
	...
	while (nHomePos < nLength && CharTraitsT::IsSpace(pszChars[nHomePos]))
		++nHomePos;
...
CPoint CCrystalTextView::WordToRight(CPoint pt)
{
	...
	while (pt.x < nLength)
	{
		if (!CharTraitsT::IsAlnum(pszChars[pt.x]) && pszChars[pt.x] != _T('_'))
			break;
		pt.x++;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
CPoint CCrystalTextView::WordToLeft(CPoint pt)
{
	...
	while (pt.x > 0)
	{
		if (!CharTraitsT::IsAlnum(pszChars[pt.x - 1]) && pszChars[pt.x - 1] != _T('_'))
			break;
		pt.x--;
	}
...
void CCrystalTextView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (point.x < GetMarginWidth())
	{
		...
		else
		{
			...
			//BEGIN SW
			// Find TCHAR pos that is the beginning of the subline clicked on
			CPoint pos;
			CharPosToPoint(m_ptCursorPos.y, m_ptCursorPos.x, pos);
...
void CCrystalTextView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_bPreparingToDrag)
	{
		...
		if (hData != NULL)
		{
			...
			COleDataSource ds;
			ds.CacheGlobalData(CF_TTEXT, hData);
...
{
	return IsClipboardFormatAvailable(CF_TTEXT);
}
...
BOOL CCrystalTextView::PutToClipboard(LPCTSTR pszText)
{
	...
	if (OpenClipboard())
	{
		...
		if (hData != NULL)
		{
			...
			bOK = SetClipboardData(CF_TTEXT, hData) != NULL;
		}
...
BOOL CCrystalTextView::GetFromClipboard(CString &text)
{
	...
	if (OpenClipboard())
	{
		HGLOBAL hData = GetClipboardData(CF_TTEXT);
