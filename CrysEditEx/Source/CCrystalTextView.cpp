...
/********************************************************************
 *
 * $RCSfile: CCrystalTextView.cpp,v $
 * $Revision$
 * $Date$
 *
 * $Author$
 *
 * $Log: CCrystalTextView.cpp,v $
 * Revision 1.32  2007/01/15 18:50:39  niteria
 ...
 *
 * Revision 1.31  2006/08/20 13:59:36  niteria
 * Fixed Bug 1420318: Find whole words does not work properly
 * Fixed Bug 1345980 (duplicate): 6.31 find whole word with 1...2 letters
 *
...
BEGIN_MESSAGE_MAP(CCrystalTextView, CView)
	...
	ON_COMMAND(ID_FILE_PRINT_PREVIEW, &CCrystalTextView::OnFilePrintPreview)
	//	Status
                                    ...
                                    BOOL bTrackScrollBar /*= TRUE*/)
{
	...
	else  if (m_nOffsetChar != nNewOffsetChar)
	{
...
void CCrystalTextView::DrawLineAttributes(CDC *pdc, CPoint ptOrigin, int nLineIndex, int *anBreaks, int nBreaks)
{
	...
	if (pList != NULL)
	{
		IMAGELISTDRAWPARAMS drawing = {sizeof(IMAGELISTDRAWPARAMS)};
		...
		while (nLength > 0)
		{
			...
			int nStartChar = std::max(attr.m_nStartPos, anIndices[0]);
			int nEndChar = std::min(attr.m_nEndPos, anIndices[nLength]);
...
void CCrystalTextView::DrawMargin(CDC *pdc, const CRect &rect, int nLineIndex)
{
	...
	if (nImageIndex >= 0)
	{
		if (m_pIcons == NULL)
		{
			...
			m_pIcons->m_hImageList = ::ImageList_LoadImage(AfxFindResourceHandle(MAKEINTRESOURCE(IDR_MARGIN_ICONS),RT_BITMAP),
			                         MAKEINTRESOURCE(IDR_MARGIN_ICONS),16,1,RGB(192,192,192),IMAGE_BITMAP,LR_CREATEDIBSECTION);
...
int CCrystalTextView::CharPosToPoint(int nLineIndex, int nCharPos, CPoint &charPoint)
{
	...
	// calculate point out of TCHAR pos
...
{
	// calculate TCHAR pos out of point
	int nLength = GetLineLength(nLineIndex);
	...
	// find TCHAR pos that matches cursor position
	int nXPos = 0;
	int nYPos = 0;
	int nCurPos = 0;
	int nTabSize = GetTabSize();
...
void CCrystalTextView::OnSize(UINT nType, int cx, int cy)
{
	...
	//BEGIN SW
	// get TCHAR position of top left visible character with old cached word wrap
	CPoint topPos;
...
HGLOBAL CCrystalTextView::PrepareDragData()
{
	...
	LPTSTR pszData = static_cast<LPTSTR>(::GlobalLock(hData));

	_tcscpy(pszData,text);
	::GlobalUnlock(hData);
...
static int FindStringHelper(LPCTSTR pszFindWhere, LPCTSTR pszFindWhat, BOOL bWholeWord)
{
	...
	for (;;)
	{
		...
		if ((pszPos > pszFindWhere && (CharTraitsT::IsAlnum(pszPos[-1]) || pszPos[-1] == _T('_'))) //look before match
		        || (pszPos + nLength <= lastChar && (CharTraitsT::IsAlnum(pszPos[nLength]) || pszPos[nLength] == _T('_')))) //look after match
		{
			//Found match is not a whole word - step one TCHAR forward, so we don't find it again
...
int CCrystalTextView::GetMarginWidth()
{
	...
	if (m_bShowLineNumbers)
	{
		...
		int nMax = static_cast<int>(ceil(log10(static_cast<double>(nLines < 10 ? 10 : nLines))));
...
TCRYSTALWORDWRAP CCrystalTextView::GetWordWrapping() const
{
	return m_WordWrapType;
}

void CCrystalTextView::SetWordWrapping(TCRYSTALWORDWRAP WrapType)
{
	m_WordWrapType = WrapType;
...
void CCrystalTextView::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	// - Therefore CharTraitsT::IsAlnum() is used here as well - it will return true for them.
	...
	if (!(_istgraph(nChar) || CharTraitsT::IsAlnum(nChar)) && !(nChar == _T(' ')) /*&& !(nChar == _T('\t'))*/)
	{
	...
	*m_pstrIncrementalSearchString += static_cast<TCHAR>(nChar);
...
void CCrystalTextView::GetSelectedText(CString &strSelection)
{
	...
	else
	{
		...
		//If the cursor is placed at the end of the line ==> step one TCHAR back.
		// (If this EOL is not catched here, it will assert in the
		//  first run of the next loop - strLine[nStartChar])
		//If the cursor is placed at the end of a word, step one TCHAR back.
		int nStartChar = ptStart.x;
		if ((nStartChar >= strLine.GetLength()) || (!CharTraitsT::IsAlnum(strLine[nStartChar])))
		{
			--nStartChar;
		}
		...
		{
			if (!CharTraitsT::IsAlnum(strLine[nStartChar]))
			{
				nStartChar++;
				break;
			}
		...
		{
			if (!CharTraitsT::IsAlnum(strLine[nEndChar]))
				break;
		}
...
{
	mouse_wheel_delta_ += zDelta;

	if (std::abs(mouse_wheel_delta_) >= WHEEL_DELTA)
	{
		...
		nScrollTo -= mouse_wheel_delta_ / WHEEL_DELTA * lines;
		...
		mouse_wheel_delta_ %= WHEEL_DELTA;
	}
...
{
	return (m_WordWrapType == WORD_WRAP_FIXEDCOLUMN);
}
...
void CCrystalTextView::OnFilePrintPreview(void)
{
